...
class Advanced extends Component {
  ...
  prepData = (apiData) => {
    ...
    apiData.prices.forEach(price => {
      // amdData.labels.push(price.id)
      amdData.datasets[0].data.push(price.value)
    })
    return amdData
  }
  createChart = (amdData) => {
    const ctx = document.querySelector('#advanced')
  ...
  render(){
    return(
      <>
        <h1>Advanced</h1>
        <canvas id="advanced" width="150" height="100"></canvas>
      </>
    )
  }
}
