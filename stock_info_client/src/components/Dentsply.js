...
class Dentsply extends Component {
  componentDidMount(){
    this.getData()
  }
  getData = () => {
  ...
  prepData = (apiData) => {
    ...
    apiData.prices.forEach(price => {
      // xrayData.labels.push(price.id)
      xrayData.datasets[0].data.push(price.value)
    })
    return xrayData
  }
  createChart = (xrayData) => {
    const ctx = document.querySelector('#dentsply')
  ...
  render(){
    return(
      <>
        <h1>Dentsply</h1>
        <canvas id="dentsply" width="150" height="100"></canvas>
      </>
    )
  }
}

export default Dentsply
