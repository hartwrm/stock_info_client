...
class Msci extends Component {
  ...
  prepData = (apiData) => {
    ...
    apiData.prices.forEach(price => {
      // msciData.labels.push(price.id)
      msciData.datasets[0].data.push(price.value)
    })
    return msciData
  }
  createChart = (msciData) => {
    const ctx = document.querySelector('#msci')
  ...
  render(){
    return(
      <>
        <h1>MSCI</h1>
        <canvas id="msci" width="150" height="100"></canvas>
      </>
    )
  }
}
