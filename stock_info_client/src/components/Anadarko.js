...
class Anadarko extends Component {
  componentDidMount(){
    this.getData()
  }
  getData = () => {
  ...
  prepData = (apiData) => {
    ...
    apiData.prices.forEach(price => {
      // apcData.labels.push(price.id)
      apcData.datasets[0].data.push(price.value)
    })
    return apcData
  }
  createChart = (apcData) => {
    const ctx = document.querySelector('#anadarko')
  ...
  render(){
    return(
      <>
        <h1>Anadarko</h1>
        <canvas id="anadarko" width="150" height="100"></canvas>
      </>
    )
  }
}

export default Anadarko
